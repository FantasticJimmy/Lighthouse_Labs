...
def hide_all_sins(string)
  if string.match(/\b\d{3}-\d{3}-\d{3}\b/)
    string.scan(/\b\d{3}-\d{3}-\d{3}\b/).map {|x|x.sub /\d{3}-\d{3}-(\d{3})/,'XXX-XXX-\1'}.join(', ')
  else
    string
  end
end
...
def format_sins(string)
  if string.split.map {|x|x.scan(/\d/).length} != [9,9,9]
    return string
  else
    arr = string.split.map {|x|x.scan(/\d/)}
    arr2 = arr.map do |x|
      x.insert(3,"-")
      x.insert(7,"-")
    end
    arr3 = arr2.map{|x|x.join("")}
    arr4 = arr3.join(", ")
  end
end
